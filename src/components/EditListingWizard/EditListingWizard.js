...
export const TABS = [DESCRIPTION, POLICY,FEATURES, SESSION,  LOCATION, PRICING, PHOTOS];
...
const tabLabel = (intl, tab) => {
  ...
  if (tab === DESCRIPTION) {
    ...
    key = 'EditListingWizard.tabLabelSession';
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    ...
    case FEATURES:
      ...
      case SESSION:
      return !!(publicData && publicData.session);
    case LOCATION:
...
class EditListingWizard extends Component {
  ...
  handleCreateListing(values) {
   ...
   const { onCreateListing } = this.props;
